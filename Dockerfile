...
RUN apk --update --no-cache --update-cache --allow-untrusted add \
    ...
    # Install Composer
    curl -sS https://getcomposer.org/installer | php -- --install-dir=/usr/bin --filename=composer --version=1.9.3 && \
    # Configure php.ini
    echo $'memory_limit = 1024M' >> /etc/php7/php.ini && \
    rm -rf /var/cache/apk/* && \
    mkdir /gitlab-composer/.composer && \
    echo "{}" > /gitlab-composer/.composer/composer.json && \
